...
import math
import io     # frank add
import socket # frank add
import struct # frank add
import time   # frank add
import pickle # frank add

client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # frank add
client_socket.connect(('10.36.162.52', 8485)) # frank add
...
def main():
    ...
    img_counter = 0 # frank add
    cap = cv2.VideoCapture(0)

    encode_param=[int(cv2.IMWRITE_JPEG_QUALITY),90] # frank add
    while True:
        ...
        result, image = cv2.imencode('.jpg', frame, encode_param) # frank add
        data = pickle.dumps(image, 0) # frank add
        size = len(data) # frank add

        if img_counter%2==0: # frank add
            client_socket.sendall(struct.pack(">L", size) + data) # > big-endian L long unsigned # frank add
            #cv2.imshow('client',frame)
            
        img_counter += 1 # frank add
